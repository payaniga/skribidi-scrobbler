...
// Modified for browser/service-worker use (no CommonJS require/module.exports)

function blake2b_normalizeInput(input) {
...
function blake2bCompress(ctx, last) {
  for (let i = 0; i < 16; i++) { _v[i] = ctx.h[i]; _v[i + 16] = BLAKE2B_IV32[i]; }
  _v[24] ^= ctx.t;
  _v[25] ^= Math.floor(ctx.t / 0x100000000);
  if (last) { _v[28] = ~_v[28]; _v[29] = ~_v[29]; }
  for (let i = 0; i < 32; i++) _m[i] = B2B_GET32(ctx.b, 4 * i);
  for (let i = 0; i < 12; i++) {
  ...
  for (let i = 0; i < 16; i++) ctx.h[i] ^= _v[i] ^ _v[i + 16];
}

const _paramBlock = new Uint8Array(64);

function blake2bInit(outlen, key) {
  if (outlen === 0 || outlen > 64) throw new Error('Illegal output length');
  _paramBlock.fill(0);
  _paramBlock[0] = outlen;
  if (key) _paramBlock[1] = key.length;
  _paramBlock[2] = 1; // fanout
  _paramBlock[3] = 1; // depth
  const ctx = { b: new Uint8Array(128), h: new Uint32Array(16), t: 0, c: 0, outlen };
  for (let i = 0; i < 16; i++) ctx.h[i] = BLAKE2B_IV32[i] ^ B2B_GET32(_paramBlock, i * 4);
  if (key) { blake2bUpdate(ctx, key); ctx.c = 128; }
  return ctx;
}

function blake2bUpdate(ctx, input) {
  for (let i = 0; i < input.length; i++) {
    if (ctx.c === 128) { ctx.t += ctx.c; blake2bCompress(ctx, false); ctx.c = 0; }
    ctx.b[ctx.c++] = input[i];
  }
}
...
function blake2b(input, key, outlen) {
  outlen = outlen || 64;
  input = blake2b_normalizeInput(input);
